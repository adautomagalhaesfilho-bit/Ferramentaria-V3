...
function desenharGeral(ini, fim) {
  ...
  // Top moldes (todos setores juntos)
  const porMolde = {};
  ...
  setTimeout(() => {
    ...
    for(let d=new Date(ini+'T12:00:00');d<=new Date(fim+'T12:00:00');d.setDate(d.getDate()+1)) {
      ...
      if(d.getDay()!==0&&d.getDay()!==6) porDia[ds]={U:0,B:0,P:0};
    }
    lancs.forEach(l => {
...
function desenharSetor(setor, ini, fim) {
  ...
  opEntries.forEach(op=>{
    ...
    const badge=op.pct>=100?`<span style="background:#d1fae5;color:#059669;font-size:10px;font-weight:700;padding:2px 7px;border-radius:10px">✅ Meta</span>`:
    ...
    html+=`<div class="barra-wrap">
      <div class="barra-header"><div class="barra-nome">${op.nome} ${badge}</div>
        <div style="display:flex;gap:12px;align-items:center"><span style="font-size:12px;color:#64748b">${fmtMin(op.mins)}</span><span class="barra-valor" style="color:${c}">${op.pct}%</span></div>
      </div><div class="barra-track"><div class="barra-fill" style="width:${Math.min(op.pct,100)}%;background:${c}"></div></div>
    </div>`;
  });
  html+=`</div>`;
...
function desenharProducao(ini,fim){
  ...
  db.buscarProdPeriodo(ini,fim,'Todas','Todos','Todos').then(prod=>{
    ...
    // MTTR — média de tempo de reparo das corretivas
    ...
    const porTec={};
    prod.forEach(p=>{
      (p.tecnicos||'').split(',').forEach(t=>{
        const tn=t.trim();if(!tn) return;
        if(!porTec[tn]) porTec[tn]={count:0,mins:0};
        porTec[tn].count++; porTec[tn].mins+=p.minutos||0;
      });
    });
